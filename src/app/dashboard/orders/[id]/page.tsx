...
export default function OrderDetailPage() {
  ...
  if (loading) {
    return (
      <>
        <PageHeader title={`Pedido #${orderId}`} />
      ...
      <>
        ...
        <div className="p-8">
          <div className="glass-card rounded-2xl p-8 border border-red-500/20 text-center">
            ...
            <p className="text-white/60">Falha ao carregar detalhes do pedido.</p>
            <p className="text-xs text-white/30 mt-1">{error || "Pedido não encontrado"}</p>
          </div>
        </div>
      </>
    );
    ...
    <>
      ...
      <div className="p-8 max-w-4xl mx-auto space-y-6">
        ...
        <div className="glass-card rounded-2xl p-8 border border-white/5 space-y-6">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-white/40">Pedido #{order.id}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <DetailRow
                label="Booster"
                value={order.booster_name || "Aguardando atribuição"}
              />
            </div>
            <div className="space-y-4">
              <DetailRow
              ...
              />
              {order.notes && <DetailRow label="Observações" value={order.notes} />}
            </div>
          </div>
        </div>
        ...
        {order.completion_image_url && (
          <div className="glass-card rounded-2xl p-8 border border-green-500/20 space-y-4">
            <h3 className="text-sm font-bold text-white/40 uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="rounded-xl overflow-hidden border border-white/10">
              ...
              <img
                src={order.completion_image_url}
                alt="Comprovante de conclusão do serviço"
        ...
        {showReviewForm && (
          <div className="glass-card rounded-2xl p-8 border border-accent-gold/20 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-bold">Avalie este serviço</h3>
            </div>

            <form onSubmit={handleSubmitReview} className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm text-white/60">Sua Avaliação</label>
